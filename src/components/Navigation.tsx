  ...
  return (
    <>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-bg-nav backdrop-blur-[20px] backdrop-saturate-[180%] border-t border-border-subtle z-sticky transition-colors duration-medium ease-comfort">
        <ul className="flex items-center justify-around list-none m-0 p-0 px-[4px] py-[6px]">
          {NAV_LINKS.map((link) => {
            ...
            return (
              <li key={link.path}>
                <NavLink
                  to={link.path}
                  className="flex flex-col items-center gap-[2px] no-underline px-[8px] py-[4px] rounded-[8px] transition-colors duration-150"
                >
                  <span className={`transition-colors duration-150 ${isActive ? 'text-accent-warm' : 'text-text-muted'}`}>
                    {navIcons[link.path]}
                  </span>
                  <span className={`font-body text-[10px] leading-tight transition-colors duration-150 ${isActive ? 'text-accent-warm font-medium' : 'text-text-muted'}`}>
                    {navLabels[link.path]}
                  </span>
                  {isActive && (
                    <motion.div
          ...
          <li>
            ...
            >
              <span className="w-[20px] h-[20px] rounded-full bg-accent-sage flex items-center justify-center text-white font-display text-[10px]">
                {displayName.charAt(0).toUpperCase()}
              </span>
