...
export function Navigation() {
  ...
  return (
    <>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-bg-nav backdrop-blur-[20px] backdrop-saturate-[180%] border-t border-border-subtle z-sticky transition-colors duration-medium ease-comfort">
        <ul className="flex items-center justify-between list-none m-0 p-0 px-[2px] py-[6px]">
          {NAV_LINKS.map((link) => {
            const isActive = location.pathname === link.path;
            return (
              <li key={link.path} className="flex-1 min-w-0">
                <NavLink
                  to={link.path}
                  className="flex flex-col items-center gap-[2px] no-underline px-[4px] py-[4px] rounded-[8px] transition-colors duration-150"
                >
                  <span className={`transition-colors duration-150 ${isActive ? 'text-accent-warm' : 'text-text-muted'}`}>
                    {navIcons[link.path]}
                  </span>
                  <span className={`font-body text-[9px] leading-tight transition-colors duration-150 truncate ${isActive ? 'text-accent-warm font-medium' : 'text-text-muted'}`}>
                    {navLabels[link.path]}
                  </span>
                  {isActive && (
                    <motion.div
          ...
          <li className="flex-1 min-w-0">
            <button
              onClick={handleSignOut}
              className="w-full flex flex-col items-center gap-[2px] px-[4px] py-[4px] rounded-[8px] transition-colors duration-150 bg-transparent border-none cursor-pointer"
            >
              ...
              <span className="font-body text-[9px] leading-tight text-text-muted truncate">
                Out
              </span>
            </button>
          </li>
